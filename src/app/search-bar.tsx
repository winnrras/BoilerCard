...
export function SearchBar() {
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex justify-end sm:mx-auto sm:w-full sm:max-w-md sm:justify-start"
    >
      <button
        type="button"
        onClick={() => setMobileOpen((v) => !v)}
        aria-label="Search"
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#948C79]/40 text-[#F4F1E8] transition hover:border-[#BFA97E] sm:hidden"
      >
        <SearchIcon />
      </button>

      <div
        className={`${mobileOpen ? "block" : "hidden"} absolute right-0 top-full z-20 mt-2 w-64 sm:static sm:z-auto sm:mt-0 sm:block sm:w-full`}
      >
        <div className="relative">
          <input
            ...
            className="w-full rounded-full border border-[#948C79]/40 bg-[#1B1912]/90 px-4 py-2 text-sm text-[#F4F1E8] placeholder:text-[#948C79] backdrop-blur-sm transition focus:border-[#BFA97E] focus:outline-none sm:bg-[#1B1912]/70"
          />
